...
//
// La vista es SIEMPRE la argentina —miles "." y decimal ","— en toda la app. Hubo un
// tiempo un prop `formato` para mostrar un solo campo al revés (el monto a reembolsar,
// que se pegaba en una billetera que exigía punto decimal); se sacó para que no haya
...
export function parseMontoInput(raw: string): { display: string; clean: string } {
  ...
  // La COMA manda siempre: es el separador decimal. El PUNTO solo cuenta como decimal
  // si es el último y tiene menos de 3 dígitos detrás (con 3 o más son los miles que
  // la vista agrega sola).
  let intPart: string
  let decPart: string | undefined
  const iComa = s.lastIndexOf(',')
  const iPunto = s.lastIndexOf('.')
  if (iComa !== -1) {
    intPart = s.slice(0, iComa)
    decPart = s.slice(iComa + 1)
  } else if (iPunto !== -1 && s.slice(iPunto + 1).replace(/\D/g, '').length < 3) {
    intPart = s.slice(0, iPunto)
    decPart = s.slice(iPunto + 1)
  } else {
...
export default function MontoInput({ value, onChange, ...rest }: Props) {
  ...
  // al portapapeles (así "97.485" se pega como "97485"). El decimal se conserva.
  // Devuelve null si no había nada que sacar (comportamiento normal del navegador).
