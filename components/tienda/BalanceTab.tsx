...
interface Row {
  ...
  orderNumber: string
  billetera: string
  // "hecho por" (quién cargó/marcó el pago) NO se expone: es trazabilidad interna,
  // se guarda en registro_log.hecho_por y no viaja al cliente ni se muestra.
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(0,212,255,0.12)', background: 'linear-gradient(135deg, #0d1117, #111827)' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="w-full text-sm" style={{ borderCollapse: 'collapse', minWidth: '900px' }}>
            ...
            <tbody>
              {loadingRows ? (
                <tr><td colSpan={admin ? 10 : 9} className="px-3 py-8 text-center text-sm" style={{ color: 'rgba(148,163,184,0.5)' }}>Cargando…</td></tr>
              ) : rows.length === 0 ? (
