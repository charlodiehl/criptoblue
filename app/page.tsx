...
export default function Dashboard() {
  ...
  // Origen de la app, para los valores que se copian a la config de Shopify. En SSR
  // no hay window: se cae al dominio de producción, que es el que va en la app real.
  const origenApp = typeof window !== 'undefined' ? window.location.origin : 'https://criptoblue.vercel.app'
            ...
            >
              ...
              {addStep === 'shopify' && (
                <>
                  ...
                  {/* Guia paso a paso. Ya no se "carga la app" desde aca: la app se crea y se
                      configura en el panel de Shopify, y este instructivo solo provee los
                      valores que hay que pegar alla. Por eso no hay ningun campo editable. */}
                  <p style={{ color: 'rgba(148,163,184,0.7)', fontSize: '13px', textAlign: 'center', marginBottom: '18px', lineHeight: 1.55 }}>
                    Shopify obliga a crear <b style={{ color: 'rgba(226,232,240,0.9)' }}>una app por cada tienda</b>. Segui estos pasos:
                  </p>
                    ...
                    {[
                      <>Entra a{' '}
                        <a href="https://partners.shopify.com" target="_blank" rel="noopener noreferrer"
                          style={{ color: '#96BF48', fontWeight: 700, textDecoration: 'underline' }}>
                          partners.shopify.com
                        </a>{' '}
                        &rarr; <b>Apps</b> &rarr; <b>Create app</b> &rarr; <b>Create app manually</b>. Ponele un nombre (ej. &quot;Automatizacion [Tienda]&quot;).</>,

                      <>En <b>Configuration</b>, carga estos valores <b>exactos</b>:
                        <div style={{ marginTop: '6px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(150,191,72,0.2)', borderRadius: '8px', padding: '8px 10px', fontSize: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <ValorCopiable etiqueta="App URL" valor={`${origenApp}/`} onError={addToast} />
                          <ValorCopiable etiqueta="Redirect URL" valor={`${origenApp}/api/shopify/callback`} onError={addToast} />
                          <ValorCopiable etiqueta="Scopes" valor="read_orders, write_orders, read_customers" onError={addToast} />
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <span style={{ color: 'rgba(148,163,184,0.6)', flexShrink: 0 }}>Embed app in Shopify admin:</span>
                            <span style={{ color: 'rgba(226,232,240,0.9)' }}>No</span>
                          </div>
                        </div>
                      </>,

                      <>En <b>Distribution</b> elegi <b>Custom distribution</b>. Te va a pedir el dominio de la tienda:
                        <div style={{ marginTop: '6px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(150,191,72,0.2)', borderRadius: '8px', padding: '8px 10px', fontSize: '12px', lineHeight: 1.6, color: 'rgba(226,232,240,0.85)' }}>
                          Ingresa a tu tienda en Shopify, anda al apartado de <b>ordenes</b> y pega la URL que te muestre tu navegador. Se deberia ver como algo asi:
                          <div style={{ marginTop: '6px' }}>
                            <code style={{ color: '#96BF48', userSelect: 'all', wordBreak: 'break-all' }}>https://admin.shopify.com/store/id_tienda/orders</code>
                          </div>
                        </div>
                      </>,

                      <>En <b>API credentials</b>, copia el <b>Client ID</b> y el <b>Client secret</b>: son los que habilitan la conexion.</>,

                      <>En <b>Distribution</b>, toca <b>Generate link</b> y mandale ese link al dueno para que <b>instale</b> la app en su tienda.</>,
                    ].map((paso, i) => (
...
// Una linea "Etiqueta: valor" con un boton para copiar el valor al portapapeles.
// Se usa en el instructivo de Shopify: esos valores hay que pegarlos EXACTOS en el
// panel de Shopify, y copiarlos a mano es donde se cuelan los errores.
function ValorCopiable({ etiqueta, valor, onError }: {
  etiqueta: string
  valor: string
  onError: (msg: string, tipo: 'success' | 'error') => void
}) {
  ...
  const copiar = async () => {
    try {
      ...
      onError('No se pudo copiar', 'error')
    }
  }

  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
      <span style={{ color: 'rgba(148,163,184,0.6)', flexShrink: 0 }}>{etiqueta}:</span>
      <code style={{ color: '#96BF48', userSelect: 'all', wordBreak: 'break-all', flex: 1, minWidth: 0 }}>{valor}</code>
      <button
        onClick={copiar}
        ...
        style={{
          ...
          background: copiado ? 'rgba(0,200,81,0.15)' : 'rgba(150,191,72,0.1)',
          border: `1px solid ${copiado ? 'rgba(0,200,81,0.5)' : 'rgba(150,191,72,0.35)'}`,
          color: copiado ? '#00c851' : '#96BF48',
        }}
      >
        {copiado ? (
