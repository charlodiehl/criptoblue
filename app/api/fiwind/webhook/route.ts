...
const LOCK_HOLDER = 'fiwind-webhook'

// POST { secret, idCoelsa, monto, nombre, fechaISO, cbuCvu, banco, raw, emailId }
//
// Recibe una transferencia de Fiwind (enviada por el Google Apps Script que lee
// el email de notificación) y la agrega a la cola de pagos sin emparejar, para
// que el auto-match la empareje con una orden por nombre + monto + fecha.
//
// Auth: secreto compartido (FIWIND_WEBHOOK_SECRET). La ruta está exenta del
// middleware de sesión (ver proxy.ts) porque la llama un servicio externo.
export async function POST(req: NextRequest) {
  try {
    const expected = process.env.FIWIND_WEBHOOK_SECRET
    if (!expected) {
      return NextResponse.json({ error: 'FIWIND_WEBHOOK_SECRET no configurado en el servidor' }, { status: 500 })
    }
    ...
    const paymentId = `fiwind-${idCoelsa}`
    ...
    try {
      ...
      const payment: Payment = {
        ...
        emailPagador: '',                      // Fiwind no informa email
        cuitPagador: '',                       // Fiwind no informa CUIT
        referencia: typeof banco === 'string' ? banco : '',
        operationId: String(idCoelsa),
        metodoPago: 'fiwind / transferencia',
        fechaPago: fechaISO,
        status: 'approved',
        source: 'fiwind',
      ...
      appendActivity(logs, 'system', 'fiwind_pago_recibido', {
